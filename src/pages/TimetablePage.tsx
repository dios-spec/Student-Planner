...
const DAY_LABELS: Record<TimetableDayKey, string> = { mon: 'Mon', tue: 'Tue', wed: 'Wed', thu: 'Thu', fri: 'Fri', sat: 'Sat' };

function todayKeyOf(): TimetableDayKey {
  const map: Record<number, TimetableDayKey> = { 1: 'mon', 2: 'tue', 3: 'wed', 4: 'thu', 5: 'fri', 6: 'sat', 0: 'mon' };
...
export default function TimetablePage() {
  ...
  const periods = ((timetable && timetable.days && timetable.days[day]) || []).slice().sort((a, b) => a.period - b.period);
    ...
    <div className="pb-24">
      ...
      <div className="space-y-2 px-4 pt-4">
        {!loading && periods.length === 0 && (
          <EmptyState emoji="📅" title="No periods yet" subtitle="Tap + to add the first period for this day." />
        )}
        {periods.map((p) => (
          <div key={p.period} className="flex items-center gap-3 rounded-2xl border border-line bg-surface p-3.5">
            ...
            <button onClick={() => { setEditingPeriod(p); setSheetOpen(true); }} aria-label="Edit period" className="rounded-full p-1.5 text-ink-soft hover:text-accent">
              <Pencil size={15} />
            </button>
            <button onClick={() => setDeleteTarget(p)} aria-label="Delete period" className="rounded-full p-1.5 text-ink-soft hover:text-coral">
              <Trash2 size={15} />
            </button>
          </div>
        ))}
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="Delete period?"
        message="This period will be removed from the timetable for everyone."
        confirmLabel="Delete"
        danger
