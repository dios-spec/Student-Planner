...
export default function TaskCard({ item, done, onToggleDone, onEdit, onDelete, showCheckbox }: TaskCardProps) {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <div
      className={`group relative rounded-2xl border border-line bg-surface p-3.5 shadow-sm transition-opacity ${
        done ? 'opacity-60' : ''
      }`}
    >
        ...
        {showCheckbox && (
          <button
            ...
            className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
              done ? 'border-success bg-success text-white animate-pop' : 'border-line text-transparent'
            }`}
          >
        ...
        <div className="min-w-0 flex-1">
          ...
          {(item.updatedByName || item.createdByName) && (
            <p className="mt-1.5 text-[11px] text-ink-soft/70">
              {item.updatedByName ? `Updated by ${item.updatedByName}` : `Added by ${item.createdByName}`}
            </p>
          )}
        </div>
        ...
        <div className="relative shrink-0">
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-8 z-20 w-36 overflow-hidden rounded-xl border border-line bg-surface shadow-lg">
                <button
                  onClick={() => {
                    setMenuOpen(false);
                    onEdit();
