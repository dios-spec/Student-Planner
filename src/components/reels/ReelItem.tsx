...
import { Heart, MessageCircle, Send, Volume2, VolumeX, Play, Trash2 } from 'lucide-react';
...
interface ReelItemProps {
  reel: Reel;
  active: boolean;         // is this the reel currently in view?
  muted: boolean;
  onToggleMute: () => void;
...
export default function ReelItem({
  reel, active, muted, onToggleMute, onOpenProfile, onOpenComments, onShare, onDelete,
}: ReelItemProps) {
  const { user } = useAuth();
  ...
  // Play only the active reel; pause the rest to save battery/bandwidth.
  useEffect(() => {
  ...
  return (
    <div className="relative h-full w-full snap-start bg-black">
      ...
      <button
        ...
        className="absolute right-4 top-[calc(env(safe-area-inset-top)+1rem)] rounded-full bg-black/40 p-2 text-white"
      >
      ...
      <div className="absolute bottom-24 right-3 flex flex-col items-center gap-5">
        ...
        <button onClick={() => onOpenComments(reel.id)} className="flex flex-col items-center gap-1 text-white" aria-label="Comments">
          <MessageCircle size={30} />
        </button>
        <button onClick={() => onShare(reel)} className="flex flex-col items-center gap-1 text-white" aria-label="Share">
          <Send size={28} />
        </button>
        {isMine && (
          <button onClick={() => onDelete(reel)} className="flex flex-col items-center gap-1 text-white" aria-label="Delete">
            <Trash2 size={26} />
          </button>
        )}
      </div>
